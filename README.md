...
| `/dev/dentist` | только dev | главная кабинета врача на моках в каркасе кабинета: `?v=many`, `few`, `empty`, `setup` |
| `/dev/patient` | только dev | карта пациента у врача на моках в каркасе кабинета: `?v=many`, `few`, `empty` |
| `/dev/requests` | только dev | заявки на моках в каркасе кабинета: `?v=many`, `one`, `empty`; смена статуса под анонимом откатывается (RLS) |
| `/dev/visit` | только dev | детали визита на моках в каркасе кабинета: `?v=treatment`, `mapping`, `long`, `child`, `bare` |
